...
namespace Remotely.Server.Hubs
{
  public class CasterHub : Hub
  {
    ...
    public async Task ReceiveDeviceInfo(string serviceID, string machineName, string deviceID)
    {
      SessionInfo.ServiceID = serviceID;
      SessionInfo.MachineName = machineName;
      SessionInfo.DeviceID = deviceID;

      // check if any viewers are waiting
      if (ViewerHub.ViewersWaitingForConnection.TryGetValue(SessionInfo.DeviceID, out var prejoinIds))
      {
